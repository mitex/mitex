...
#define VERSION "2.7 of March 26, 2010."
...
Revision history:
	...
	- Small enhancements contributed by Lion Vollnhals
	  ...
	  - Changed translation of ¦ and ` to match the corresponding html renderings better.
	080223
...
#define DEBUG_P2(X,A1,A2)  printf(X,A1,A2)
#define DEBUG_P3(X,A1,A2,A3)  printf(X,A1,A2,A3)
#define DEBUG_P4(X,A1,A2,A3,A4)  printf(X,A1,A2,A3,A4)
#define DEBUG_P5(X,A1,A2,A3,A4,A5)  printf(X,A1,A2,A3,A4,A5)
#define BREAK { int i = i/0; } /* sneaky trick to remain in debugger */
#else
#ifdef DYN_DEBUG
bool option_debug = FALSE;
#define DEBUG_PRINT(X)   if (option_debug) printf X
#define DEBUG_P(X)  if (option_debug) printf(X)
#define DEBUG_P1(X,A1)  if (option_debug) printf(X,A1)
#define DEBUG_P2(X,A1,A2)  if (option_debug) printf(X,A1,A2)
#define DEBUG_P3(X,A1,A2,A3)  if (option_debug) printf(X,A1,A2,A3)
#define DEBUG_P4(X,A1,A2,A3,A4)  if (option_debug) printf(X,A1,A2,A3,A4)
#define DEBUG_P5(X,A1,A2,A3,A4,A5)  if (option_debug) printf(X,A1,A2,A3,A4,A5)
#define BREAK { int i = i/0; } /* sneaky trick to remain in debugger */
#else
#define DEBUG_PRINT(X)
#define DEBUG_P(X)  
#define DEBUG_P1(X,A1)  
#define DEBUG_P2(X,A1,A2)  
#define DEBUG_P3(X,A1,A2,A3)  
#define DEBUG_P4(X,A1,A2,A3,A4) 
#define DEBUG_P5(X,A1,A2,A3,A4,A5) 
...
#define DO_DEBUG_PRINT(X)   printf X
#define DO_DEBUG_P(X)  printf(X)
#define DO_DEBUG_P1(X,A1)  printf(X,A1)
#define DO_DEBUG_P2(X,A1,A2)  printf(X,A1,A2)
#define DO_DEBUG_P3(X,A1,A2,A3)  printf(X,A1,A2,A3)
#define DO_DEBUG_P4(X,A1,A2,A3,A4)  printf(X,A1,A2,A3,A4)
...
void init_opts()
{
	...
	assign_str(&opt_t_img    , "\\fbox{\\tt %n %mR}%tR");
...
{   file_t *tfile = the_files;
	...
	DEBUG_PRINT(("name_referenced(%s, %s)\n", file, name));
...
*/
{
	...
	DEBUG_PRINT(("check_ref(%s,%s,%s,%s) %d\n",
...
{   file_t *file;
	...
	for (file = the_files; file != NULL; file = file->next)
		...
		{   link_t *link;
			...
			DEBUG_PRINT(("make_by_refs for `%s'\n", file->name));
...
{   file_t *file;
	...
	for (file = the_files; file != NULL; file = file->next)
		...
		{   link_t *link;
			...
			DEBUG_PRINT(("file_re_name for `%s'\n", file->name));
...
/* Step 1: */
	...
	{   int i = strlen(origin);
		...
		DEBUG_PRINT(("glue %s with %s", origin, file));
...
} ch_table[NR_CH_TABLE] =
{
  ...
  /*164*/ { NULL,     NULL,       '\0' },  
  /*165*/ { NULL,     NULL,       '\0' },  /* Yen */
  /*166*/ { NULL,     "{\\fontencoding{U}\\fontfamily{wasy}\\selectfont\\char124}", '\0' },
  /*167*/ { "sect",   "{\\S}",      '\0' },
  /*168*/ { NULL,     "{\\tt{}\"{}}", '\0' },
  /*169*/ { "c",      "\\copyright{}", 'c' },
  /*170*/ { NULL,     NULL,       '\0' },
  ...
  /*255*/ { "yuml", "\\'y",       'y' },
		  ...
		  { "sup2",   "${}^2",             0 },
		  { "sup3",   "${}^3",             0 },
		  { "euro",   "{\\fontencoding{U}\\fontfamily{mvs}\\selectfont\\char164}",  0 },
		  { "brvbar", "{\\fontencoding{U}\\fontfamily{wasy}\\selectfont\\char124}", 0 },
};
...
{   int j;
	...
	if (found && tags[tagnr].kind == T_CHAR) 
...
{   FILE *fin,
	...
	{   int i;
		...
		if (error)
		{   printf("Usages: html2tex [options] <file>\n");
			printf("\nOptions:\n");
