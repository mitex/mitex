...
typedef enum {
  s_quit, s_normal, s_title, s_header, s_list, s_describing, s_verbatim, s_mathverbatim
} state_t;
  ...
  ListToLaTeX(FILE *fpout, struct mark_up *mptr, state_t *state)
{
  ...
  while (current != s_quit && mptr) {
	...
	fprintf(fpout, "%s", text);
      else if(current == s_mathverbatim)
	fprintf(fpout, "%s", strtok(text,"`"));
      else
...
void main(int argc, char **argv)
{
  ...
  do {
    ...
    /* write LaTeX headers and title pages, etc */ /*REMOVED - HEADER NOT NEEDED [tana]*/
    /*
    fprintf(fpout, "%s", ID);
    ...
    */
    ...
    /* write trailers */ /*NO NEED EITHER [tana]*/
    /*
    if (option[o_footer])
